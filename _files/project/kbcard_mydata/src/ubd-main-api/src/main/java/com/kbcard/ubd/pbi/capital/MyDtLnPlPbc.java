 ...
 * 
 *
 * @logicalName   마이데이터 대출상품계좌추가정보조회Pbi
 * @lastDate      2021-05-31
 */
public class MyDtLnPlPbc {
	 ...
	 * 
	 */
	public LData retvMyDtLnPlInf( LData input ) throws LException {
		LLog.debug.println( input );
		LLog.debug.println( " ☆☆☆ 마이데이터대출상품계좌추가정보조회 pbi ☆☆☆ START " );
		...
		try {
			...
			LLog.debug.println( "마이데이터대출상품계좌추가정보조회 pbi 거래중 오류 발생 LException ☆☆☆" );
	...
	private LData verifyIn(LData input) throws LException {
		...
		iDupDmd.setString("거래구분코드_V6", UBD_CONST.API_DTCD_CAPITAL_BAS_INF_INQ); // API구분코드(대출상품목록조회)
	...
	class MyDtLnPdAccBasInfInqCpbc{
		...
		public LData retvLnPdAccBasInfLnkd(LData input) throws LException {
			...
			try {
				...
				if( (intfDtcd.equals("16") || intfDtcd.equals("38")) ) { //카드론
					bwkDtcd = "카드론"; 
					intfId = "UBD_2_KIWS00003";
					lnkdDc = "EAI";
					result = retvEaiLnPdAccBasInf(input); //EAI대출상품계좌추가조회 (카드론)
				}else { //할부금융
					bwkDtcd = "할부금융"; 
					intfId = "UBD_1_GEAS00004";
					lnkdDc = "MCI";
					result = retvMciLnPdAccBasInf(input); //MCI대출상품계좌추가조회 (할부금융)
				}
				...
				rUbdCommon.setString("API거래코드", "UBD2000540");
		...
		private LData retvEaiLnPdAccBasInf( LData input ) throws LException {
			...
			rEai = httpClient.sendOutboundMessage(TargetTypeConst.EAI, "UBD_1_GEAS00004", iEai); // 대출상품 계좌 기본정보 조회 (카드론)
			...
			result.setString("통화코드", rEai.getString("통화코드") );
			result.setInt("대출잔액", rEai.getInt("대출잔액") );
			result.setInt("대출원금", rEai.getInt("대출금액") );
			result.setString("다음이자상환일", rEai.getString("다음납입년월일") );
			
			LLog.debug.println( " ☆☆☆ 카드론 EAI대출상품계좌추가정보조회 cpbi ☆☆☆ END " );
			LLog.debug.println( result );
		...
		private LData retvMciLnPdAccBasInf( LData input ) throws LException {
			...
			rMci = httpClient.sendOutboundMessage(TargetTypeConst.MCI, "UBD_2_KIWS00003", iMci); // 대출상품 계좌 추가정보 조회 (할부금융)
			linkResponseHeader = (LData) ContextHandler.getContextObject(LContextKey.LINK_SYSTEM_HEADER);
			...
			if (!sdtErrMsgCd.equals("N0000000")) {
				LLog.debug.println("EAI 대출상품계좌추가본정보조회 오류 발생 LBizException ☆☆☆");
				LLog.debug.println("e.getcode ☆☆☆", sdtErrMsgCd);
				rUbdCommon = mydtCstPntRmgInqCpbc.isidTrErrMsgInq(sdtErrMsgCd, "MCI");
			}
			
			result.setString("통화코드", rMci.getString("통화코드") );
			result.setInt("대출잔액", rMci.getInt("대출잔액_N18_3") );
			result.setInt("대출원금", rMci.getInt("대출원금금액") );
			result.setString("다음이자상환일", rMci.getString("다음납입년월일") );
			
			LLog.debug.println( " ☆☆☆ 할부금융 MCI대출상품계좌추가정보조회 cpbi ☆☆☆ END " );
			LLog.debug.println( result );
	...
	private LData reset_Rsp_Ldata(LData input) {
		...
		result.setString("통화코드", input.getString("통화코드") );
		result.setInt("대출잔액", input.getInt("대출잔액") );
		result.setInt("대출원금", input.getInt("대출원금") );
		result.setString("다음이자상환일", input.getString("다음이자상환일") );
	...
	private LData reset_Rsp_Mci_Ldata(LData input) {
		...
		result.setString("대출회차_V3"		, input.getString("대출회차_V3"));
		result.setString("통화코드", input.getString("통화코드") );
		result.setInt("대출잔액", input.getInt("대출잔액_N18_3") );
		result.setInt("대출원금", input.getInt("대출원금금액") );
		result.setString("다음이자상환일", input.getString("다음이자상환일") );
