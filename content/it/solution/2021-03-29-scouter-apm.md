...
Client는 Client툴을 이용하여 Host서버에 붙어 모니터링 정보를 확인 할 수 있다.  
Client ↔︎ Collector(EC2) ↔︎ WAS(VM/Container)  

<img src="/assets/it/solution/scouter/scouter.png" alt="scouter" style="width: 50%; height: auto;">  


Scouter의 동작을 확인하기 위해서는 즉, 3가지 설치가 필요하다. (Linux 환경을 기본으로 한다.)
<br><br>

#### Agent (scouter/agent.java)  

scouter/agent.java의 구성은 conf, plugin, scouter.agent.jar로 구성이 되어있다.  
...
<img src="/assets/it/solution/scouter/scouter1.png" alt="scouter1" style="width: 40%; height: auto;">  

Host서버의 IP주소를 입력한다.  
초기 계정 및 패스워드는 admin / admin으로 로그인 한다.  
물론 Client PC와 Host 간 6100 TCP 통신은 허용이 되어 있어야 정상 접근이 가능할 것이다.  
