...
![scouter](/assets/it/project/koreanair/koa7.png){: width="60%" height="auto"}  
...
env_valut.sh을 아래와 같이 "attribute_name" "암호화할 패스워드"로 실행하면 빨간 박스로 강조한 두 값을 복사한다.(attribute_name은 keystore 별로 unique)  
```bash
./env_valut.sh application_name koa123!
```

![scouter](/assets/it/project/koreanair/koa10.png){: width="90%" height="auto"}  
