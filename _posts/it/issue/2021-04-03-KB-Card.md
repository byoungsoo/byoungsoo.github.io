...
Pod에서 MCI/EAI 연계서버를 통해 EAI를 호출 할 때 Timeout Exception이 발생하였음  
확인 결과 Pod의 서버시간과 EAI쪽 서버시간의 동기화가 맞지 않았고, Pod의 서버시간은 실제 인터넷 시간과도 많게는 40초 작게는 15초 정도의 Gap이 있었음  

같은 Workernode 서버에 스케쥴링된 Pod의 시간은 똑같았고, 각 다른 Workernode서버에서 기동 된 Pod의 시간은 달랐다.  
Pod의 시간은 Node의 시간 설정을 따를 것으로 보였고, 다른 WAS서버들의 서버시간도 모두 온라인 시간과 틀어져있었다.  

결과적으로는 모든 EC2 서버가 KB카드 쪽의 타임서버와 시간이 동기화가 되지 않고 있었음을 알았고, AWS 타임서버로 변경하여 작업 완료 하였음.  

chrony 데몬이 정상동작을 하고 있어야 하며 설정파일은 /etc/chrony.conf에 존재한다.  
```bash
ps -ef | grep chrony
/etc/chrony.conf
169.254.169.123 (AWS Timeserver)
```
