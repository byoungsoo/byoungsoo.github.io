...
개발 소스에서 Producer.send()를 할 때마다 new Thread()를 생성하여 Thread가 지속생성 된 것으로 확인 되었으며 해당 소스를 조치하고 나서 OOMKilled 되는 현상과 아래의 현상이 모두 조치가 되었다.  
...
이유는 Pod가 초기 기동 될 때 사용되는 메모리양인데 
